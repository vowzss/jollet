...
namespace jollet::types
{
    struct jvalue {
      ...
      public:
        ...
        template <typename T, std::enable_if_t<!std::is_arithmetic_v<T>, int> = 0> const T* try_as() const {
            return std::get_if<T>(&value_);
        }

        template <typename T, std::enable_if_t<!std::is_arithmetic_v<T>, int> = 0> const T& as() const {
